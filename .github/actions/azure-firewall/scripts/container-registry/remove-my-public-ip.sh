...
function main() {
    ...
    # Remove public IP address from Container Registry firewall rules
    az acr network-rule remove \
        --name "$containerregistry_name" \
        --resource-group "$resource_group_name" \
        --ip-address "$public_ip" \
        --output none || {
        pretty_print "Error: Failed to remove public IP address: $public_ip from Container Registry firewall rules. Exiting." "$bg_red" >&2
        exit 1
    }

    # Print success message

    pretty_print "Successfully removed your public IP address: $public_ip from Container Registry firewall rules." "$fg_green"
